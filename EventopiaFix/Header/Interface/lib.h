...
void getinputmin(char input[], int min, int max, int type){
    ...
    do
    {
        back:
        strcurrent = getch();
        if(strcurrent == UP_KEY || strcurrent == LEFT_KEY || strcurrent == RIGHT_KEY || strcurrent == DWN_KEY){
            goto back;
        }
...
void rupiah(int number, char output[]) {
    ...
    if(number < 1000) {
        ...
        sprintf(output, "%d.%03d.%03d", number/1000000, (number%1000000)/1000, number%1000);
    } else {
        sprintf(output, "%d.%03d.%03d.%03d", number/1000000000, (number%1000000000)/1000, (number%1000000)/1000, number%1000);
    }
}
